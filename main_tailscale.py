...
class App(ctk.CTk):
        ...
        if os.path.exists(config_path):
            try:
                with open(config_path, "r") as f:
                    c = json.load(f)
                    if c.get("mode") == "host":
                        network.host_database(c.get("db_name", "Baza"))
                        mode = "host"
                    elif c.get("mode") == "client":
                        network.connect_to_database(c.get("server_ip"))
                        mode = "client"
            except:
                pass
    ...
    def on_closing(self):
        ...
        if is_host:
            # Hide the window and start tray icon
            self.withdraw()
            import pystray
            ...
            d.text((10,10), "Hokimiyat", fill=(255,255,255))
            ...
            menu = pystray.Menu(
                pystray.MenuItem("Dasturni ochish", show_window, default=True),
                pystray.MenuItem("Butunlay yopish", quit_window)
            )
            
            self.tray_icon = pystray.Icon("name", image, "Hokimiyat Boshqaruv Tizimi", menu)
            import threading
    ...
    def build_network_ui(self):
        self.net_container = ctk.CTkFrame(self, fg_color="transparent")
        self.net_container.grid(row=0, column=0, sticky="nsew")
        self.net_container.grid_rowconfigure(0, weight=1)
        self.net_container.grid_columnconfigure(0, weight=1)
        
        card = ctk.CTkFrame(self.net_container, corner_radius=15, width=600, height=580, fg_color="#1e293b")
        card.grid(row=0, column=0)
        card.grid_propagate(False)
        card.grid_columnconfigure(0, weight=1)
        
        ctk.CTkLabel(card, text="Tarmoq Sozlamalari (Tailscale/LAN)", font=("Roboto", 24, "bold"), text_color="white").pack(pady=(25, 15))
        
        # Host section
        ctk.CTkLabel(card, text="Yangi Baza Yaratish (Siz Server bo'lasiz)", font=("Roboto", 16)).pack(pady=5)
        
        def create_db():
            db_name = simpledialog.askstring("Baza nomi", "Yangi baza uchun nom kiriting (Masalan: Viloyat Hokimligi):", initialvalue="Hokimiyat Bazasi")
            if db_name:
                network.host_database(db_name)
                with open(network.CONFIG_PATH, "w") as f:
                    json.dump({"mode": "host", "db_name": db_name}, f)
                self.net_container.grid_forget()
                self.start_app()
                
        ctk.CTkButton(card, text="Yangi Baza Yaratish", command=create_db, height=40, fg_color="#059669", font=("Roboto", 16)).pack(pady=(0, 15))
        
        ctk.CTkFrame(card, height=2, fg_color="#334155").pack(fill="x", padx=40, pady=5)
        
        # Join section
        ctk.CTkLabel(card, text="Mavjud Bazaga Qo'shilish (Tarmoqdan va saqlanganlar)", font=("Roboto", 16)).pack(pady=(10, 5))
        
        list_frame = ctk.CTkScrollableFrame(card, fg_color="#0f172a", corner_radius=10, height=180)
        list_frame.pack(fill="x", padx=40, pady=10)
        
        saved_servers_path = os.path.join(network.DATA_DIR, "saved_servers.json")

        def load_saved_servers():
            if os.path.exists(saved_servers_path):
                try:
                    with open(saved_servers_path, "r", encoding="utf-8") as sf:
                        return json.load(sf)
                except: pass
            return []

        def save_manual_ip(ip, name=None):
            saved = load_saved_servers()
            srv_name = name or f"Saqlangan Baza ({ip})"
            # Update or append
            filtered = [s for s in saved if s.get('ip') != ip]
            filtered.append({"name": srv_name, "ip": ip})
            try:
                with open(saved_servers_path, "w", encoding="utf-8") as sf:
                    json.dump(filtered, sf, ensure_ascii=False, indent=2)
            except: pass

        def delete_saved_ip(ip):
            saved = load_saved_servers()
            filtered = [s for s in saved if s.get('ip') != ip]
            try:
                with open(saved_servers_path, "w", encoding="utf-8") as sf:
        ...
        def connect_db(ip):
            network.connect_to_database(ip)
            with open(network.CONFIG_PATH, "w") as f:
                json.dump({"mode": "client", "server_ip": ip}, f)
            self.net_container.grid_forget()
            self.start_app()
            
        def show_servers(discovered_servers):
            ...
            for s in saved:
                if s['ip'] not in all_ips:
                    combined.append(s)
                    all_ips.add(s['ip'])
            
            if not combined:
                ctk.CTkLabel(list_frame, text="Tarmoqda yoki saqlanganlarda faol bazalar topilmadi.", text_color="gray").pack(pady=20)
            else:
                saved_ips = set(s['ip'] for s in saved)
                for s in combined:
                    row_f = ctk.CTkFrame(list_frame, fg_color="transparent")
                    row_f.pack(fill="x", pady=2)
                    
                    is_saved = s['ip'] in saved_ips
                    btn_txt = f"⭐ {s['name']} ({s['ip']})" if is_saved else f"🌐 {s['name']} ({s['ip']})"
                    
                    btn = ctk.CTkButton(row_f, text=btn_txt, command=lambda ip=s['ip']: connect_db(ip), fg_color="#3b82f6" if not is_saved else "#2563eb", hover_color="#1d4ed8", anchor="w")
                    btn.pack(side="left", fill="x", expand=True)
                    
                    if is_saved:
                        del_btn = ctk.CTkButton(row_f, text="❌", width=30, height=30, fg_color="#ef4444", hover_color="#dc2626", command=lambda ip=s['ip']: delete_saved_ip(ip))
                        del_btn.pack(side="right", padx=(5, 0))
                    
        def connect_manual():
            ip = simpledialog.askstring("Qo'lda kiritish", "Server IP manzilini kiriting (masalan: 100.x.x.x):")
            if not ip: return
            ip = ip.strip()
            if not ip: return
            save_manual_ip(ip)
        ...
        def do_search():
            servers = network.discover_servers()
            self.after(0, lambda: show_servers(servers))
            
        btn_search = ctk.CTkButton(card, text="Qidirish", command=lambda: threading.Thread(target=do_search, daemon=True).start(), height=35)
        btn_search.pack(pady=5)
        
        btn_manual = ctk.CTkButton(card, text="IP orqali ulanish va saqlash", command=connect_manual, height=35, fg_color="#f59e0b", hover_color="#d97706")
    ...
    def build_login_ui(self):
        ...
        def disconnect_network():
            if os.path.exists(network.CONFIG_PATH):
                os.remove(network.CONFIG_PATH)
            self.login_container.grid_forget()
            self.build_network_ui()
            
        ctk.CTkButton(card, text="Tarmoqni almashtirish", command=disconnect_network, fg_color="#475569", width=280, height=35, corner_radius=10).grid(row=6, column=0, pady=(0, 20))
    ...
    def build_dashboard_content(self):
        ...
        if getattr(self, 'is_host', False):
            ip_info_frame = ctk.CTkFrame(self.dash_frame, fg_color="#1e293b", corner_radius=10)
            ip_info_frame.grid(row=0, column=0, sticky="ew", padx=30, pady=(20, 0))
            
            ctk.CTkLabel(ip_info_frame, text="Siz asosiy serversiz. Boshqa kompyuterlar ulanishi uchun IP manzilingiz:", font=("Roboto", 12, "bold"), text_color="#cbd5e1").pack(side="top", pady=(10, 0))
            ctk.CTkLabel(ip_info_frame, text=network.get_all_ips(), font=("Roboto", 16, "bold"), text_color="#10b981").pack(side="top", pady=(5, 10))
